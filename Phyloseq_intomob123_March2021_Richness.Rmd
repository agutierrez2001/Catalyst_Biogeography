...
```{r}
  ...
  tax_mat<- read_excel("TAX_table.xlsx")
   samples_df <- read_excel("Metadata/samples_df_MLD_March2021_RANKED.xlsx")
...
sample_variables(intomob123_15April2021) 
 ...
 ## Identify factors within each variable     
        unique(get_variable( intomob123_15April2021, sample_variables( intomob123_15April2021)[5]))
...
CairoPDF(file = "R.Figures/Richness/Protist.Diversity_ALL_Latitude1_Area2_March2021", width = 10, height = 5) 

  plot_richness(ps[["EEZ"]], x="RankLat1", color="Area2", measures = c("Observed","Shannon"), title = "Protist ALL - Latitude") + geom_boxplot(alpha= 0.8, outlier.size = 0.2) +
                scale_color_manual(values = Area_colors) +
        geom_point(size = 0.1) +
        theme_bw() +
        #scale_x_continuous(limits = c(0, 10)) +
